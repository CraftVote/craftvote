<div class="row">
    <div class="col-xs-9">
        <h2><!--H_TITLE--></h2>
        <small class="text-muted">Зарегестрирован <span class="time"><!--REG--></span></small>
    </div>
    <div class="col-xs-3 text-right">
        <br><small class="text-muted padding-right"><span class="glyphicon glyphicon-heart"></span> <!--VOTES--></small> <button onclick="voteProject('<!--ID-->');" class="btn btn-success">Голосовать</button>
    </div>
</div>
<hr>
<img src="/res/img/projects/<!--LOGO-->" class="img-responsive img-rounded pull-center">
<hr>
<p class="text-justify"><!--DESC--></p>
<hr>
<ul class="nav nav-tabs">
  <li role="presentation" class="active"><a href="#">Основная информация</a></li>
  <li role="presentation"><a href="#">Сервера</a></li>
</ul>
<div class="nav-container">
    <table>
        <tr>
            <td><p><small class="text-muted">Владелец</small></p></td>
            <td class="padding-left"><p><small><a href="/profile/<!--USER_ID-->"><!--USER_NAME--></a></small></p></td>
        </tr>
        <tr>
            <td><p><small class="text-muted">Вебсайт</small></p></td>
            <td class="padding-left"><p><small><a target="_blank" href="<!--WEBSITE-->"><!--WEBSITE--></a></small></p></td>
        </tr>
        <tr>
            <td><p><small class="text-muted">Группа в соцсети</small></p></td>
            <td class="padding-left"><p><small><a target="_blank" href="<!--SN-->"><!--SN--></a></small></p></td>
        </tr>
    </table>
</div>